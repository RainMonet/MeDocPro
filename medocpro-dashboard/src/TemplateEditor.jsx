...
const AIEnhancementPanel = ({ isVisible, onClose, selectedText, onAddZone }) => {
  const [enhancementType, setEnhancementType] = useState('clinical');
  ...
  const handleAddZone = () => {
    if (!selectedText.trim()) {
      alert('Please select text in the template first');
      return;
    }
  ...
  return (
    <div className="card" style={{ 
      ...
      <div className="card-header" style={{ background: 'linear-gradient(135deg, #9333ea 0%, #7c3aed 100%)', color: 'white' }}>
        <div>
          <h3 className="card-title">AI Enhancement Zone Configuration</h3>
          <p style={{ fontSize: '12px', opacity: 0.9 }}>
            Configure how selected text will be enhanced when templates are populated
          </p>
        </div>
      </div>
      ...
      <div className="card-content">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px', marginBottom: '20px' }}>
          {['clinical', 'narrative', 'diagnostic'].map(type => (
            <button
              key={type}
              className={`btn btn-sm ${enhancementType === type ? 'btn-primary' : 'btn-secondary'}`}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
          <div>
            <label className="form-label">Enhancement Intensity</label>
            <input
              type="range"
              min="0"
              max="100"
            ...
            <div style={{ fontSize: '11px', color: '#64748b', marginTop: '4px' }}>
              {intensity}% - {intensity <= 25 ? 'Minimal' : intensity <= 50 ? 'Moderate' : intensity <= 75 ? 'Strong' : 'Maximum'} Enhancement
            </div>
          </div>
          <div>
            <label className="form-label">Enhancement Style</label>
            <select
              className="form-input"
              value={style}
            ...
            >
              ...
              <option value="empathetic">Patient-Centered</option>
              <option value="objective">Objective & Factual</option>
            </select>
          </div>
        </div>
        ...
        {selectedText && (
          <div style={{ 
            ...
            <div style={{ fontSize: '13px', fontStyle: 'italic', color: '#4a5568' }}>
...
const TemplatePreview = ({ content, placeholders, showSampleData }) => {
  const [processedContent, setProcessedContent] = useState('');

  useEffect(() => {
    if (!content.trim()) {
      setProcessedContent('Start typing in the template editor to see a preview here...');
      return;
    }
    ...
    if (showSampleData) {
      ...
      placeholders.forEach(placeholder => {
        ...
        processed = processed.replace(regex, placeholder.example);
      });
    } else {
      ...
      placeholders.forEach(placeholder => {
        ...
        processed = processed.replace(regex, 
          `<span style="background: rgba(102, 126, 234, 0.2); border-radius: 3px; padding: 2px 4px; margin: 0 2px; font-weight: 600; color: #0066cc;">{{${placeholder.key}}}</span>`
        );
      });
    }

    setProcessedContent(processed);
  }, [content, placeholders, showSampleData]);
...
const TemplateEditor = ({ initialTemplate, onSave, onCancel }) => {
  ...
  const [showPreviewSampleData, setShowPreviewSampleData] = useState(false);
  ...
  const addAIZone = useCallback((zone) => {
    setTemplate(prev => ({
      ...prev,
      aiEnhancementZones: [...prev.aiEnhancementZones, zone]
    }));
  }, []);
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
      ...
      <div className="card" style={{ marginBottom: '24px' }}>
        <div className="card-header">
          ...
          <p style={{ color: '#64748b', fontSize: '14px' }}>
            Create and manage HIPAA-compliant clinical documentation templates with custom placeholders
          </p>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '16px' }}>
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            <button className="btn btn-secondary" onClick={() => setTemplate({
              name: '',
              category: 'progress',
              version: '1.0',
              content: '',
      ...
      <div className="card" style={{ marginBottom: '16px' }}>
        ...
        <textarea
          ...
          placeholder="Begin creating your clinical template here. Use {{placeholder_name}} syntax to insert dynamic fields. Select text and click 'Mark for AI' to designate areas for AI enhancement..."
      ...
      <div className="card" style={{ marginBottom: '16px' }}>
        <div className="card-header">
          ...
          <button 
            ...
            onClick={() => setShowPreviewSampleData(!showPreviewSampleData)}
          >
            {showPreviewSampleData ? 'Hide Sample Data' : 'Show Sample Data'}
          </button>
        </div>
      </div>
      <TemplatePreview
